...
import { Activity, ConciergeBell, Edit3, Map, PauseCircle, ShieldCheck, SmartphoneIcon } from "lucide-react";
    ...
    const generateStatCard = (label: string, val: string, change: string, icon: string, textColor: string, bgColor: string) => {
        return (
            <div className="theme-card bg-input border border-custom rounded-lg p-4 flex flex-col justify-between h-32 hover:border-accent transition-colors">
                ...
                <div>
                    <div className="theme-layer-primary text-2xl font-bold text-main tracking-tight">{val}</div>
                    <div className="theme-layer-secondary text-xs font-medium">{label}</div>
                </div>
            </div>
        )
    }
        ...
        <div className="t-max-h-screen-header p-9 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
                <div className="theme-card cursor-pointer group relative overflow-hidden">
                    ...
                    <h3 className="theme-layer-secondary font-bold text-main text-lg">Hotel Services</h3>
                ...
                <div className="theme-card bg-card p-6 radius-lg border border-custom shadow-sm cursor-pointer group relative overflow-hidden">
                    ...
                    <h3 className="theme-layer-secondary font-bold text-main text-lg">Explore Local</h3>
                ...
                <div className="theme-card bg-card p-6 radius-lg border border-custom shadow-sm cursor-pointer group relative overflow-hidden">
                    ...
                    <h3 className="theme-layer-secondary font-bold text-main text-lg">Activity Data</h3>
                ...
                <div className="theme-card bg-card p-6 radius-lg border border-custom shadow-sm cursor-pointer group relative overflow-hidden">
                    ...
                    <h3 className="theme-layer-secondary font-bold text-main text-lg">Admin Users</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                <div className="theme-card lg:col-span-2 bg-card border border-custom radius-lg p-8 shadow-sm">
                    <div className="flex justify-between items-center mb-8">
                        <div>
                            <h3 className="theme-layer-secondary font-bold text-main text-lg">Weekly Occupancy</h3>
                            <p className="text-xs text-sub">Live room availability data</p>
                        </div>
                        <select className="bg-input text-xs font-bold p-2 radius-md text-main outline-none focus:border-accent"><option>This Week</option><option>Last Week</option></select>
                    </div>
                    <div className="h-64 w-full flex items-end justify-between gap-6 px-4 border-b border-custom pb-4 relative">
                        ...
                        {[45, 68, 52, 88, 96, 72, 80].map((h, i) => (
                            <div key={h} className="flex-1 flex flex-col items-center gap-3 group cursor-pointer z-10 relative h-full justify-end">
                ...
                <div className="theme-card bg-card border border-custom radius-lg p-6 shadow-sm flex flex-col h-full">
                    <h3 className="theme-layer-secondary font-bold text-main text-lg mb-6">Live Operations</h3>

                    <div className="flex-1 overflow-y-auto pr-2 custom-scroll max-h-[300px]">
                        <div className="p-4 bg-panel radius-md flex items-center gap-3 hover:border-accent transition-colors cursor-pointer">
                    ...
                    <button className="mt-6 w-full py-3 bg-panel text-accent text-sm font-bold radius-md hover:bg-accent hover:text-white transition-all shadow-sm">
                        View All Tasks
                    </button>
                </div>
            </div>
